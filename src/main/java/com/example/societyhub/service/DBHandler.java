...
//        return residents;
//    }
////    public Resident getResidentDetails(int sid) throws SQLException {
////    //    List<Map<String, String>> residents = new ArrayList<>();
////        Resident resident = null;
////        try (Connection connection = connectDB()) {
////            String query = "select mem_id, name, room_no, mr_ms, gender, age, contact_no from resident where sid = ?";
////            try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
////                // Set the value for the 'sid' parameter
////                preparedStatement.setInt(1, sid); // Assuming 'sid' is of type int, adjust if it's another type
////                // Now execute the query
...
////                    while (resultSet.next()) {
////    //                    Map<String, String> resident = new HashMap<>();
////    //                    resident.set("mem_id", resultSet.getString("mem_id"));
////                        assert false;
////                        resident.setName(resultSet.getString("name"));
////                        resident.setRoom_no(resultSet.getString("room_no"));
////                        resident.setMr_ms(resultSet.getString("mr_ms"));
////                        resident.setGender(resultSet.getString("gender"));
////                        resident.setAge(resultSet.getString("age"));
////                        resident.setContactNo(resultSet.getString("contact_no"));
////    //                    residents.add(resident);
////                    }
////                }
...
////    }
//    public Resident getResidentDetails(int sid) throws SQLException {
//        Resident resident = new Resident(); // Initialize the resident object
//        boolean residentFound = false; // Flag to check if resident details are found
//
//        try (Connection connection = connectDB()) {
//            String query = "select mem_id, name, room_no, mr_ms, gender, age, contact_no, mygate_no, email, bhk from resident where sid = ?";
//            try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
...
//
//                try (ResultSet resultSet = preparedStatement.executeQuery()) {
//                    if (resultSet.next()) { // Check if at least one row is returned
//                        residentFound = true; // Mark that resident details are found
//                        resident.setName(resultSet.getString("name"));
//                        resident.setRoom_no(resultSet.getInt("room_no"));
//                        resident.setMr_ms(resultSet.getString("mr_ms"));
//                        resident.setGender(resultSet.getString("gender"));
...
//        }
//
//        // If no resident was found, return null
//        return residentFound ? resident : null; // Return resident if found, otherwise null
//    }
//
//
...
//        String query = "insert into resident (mem_id, sid, name, room_no, mr_ms, gender, age, contact_no, isadmin, mygate_no, bhk, email, status) values (?, ?, ?, ?, ?, ?, ?, ?, false, ?, ?, ?, ?)";
//
...
//        String insertQuery = "INSERT INTO bill (sid, maintenance_contribution, housing_board_contribution, property_tax_contribution, sinking_fund, reserve_mhada_service_charge, sub_charge, fine, building_dev_fund, other) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
//        String updateQuery = "UPDATE bill SET maintenance_contribution = ?, housing_board_contribution = ?, property_tax_contribution = ?, sinking_fund = ?, reserve_mhada_service_charge = ?, sub_charge = ?, fine = ?, building_dev_fund = ?, other = ? WHERE sid = ?";
//
//        try (Connection connection = connectDB()) {
//            // Check if a bill already exists for the given sid
...
//                        updateStatement.setInt(10, bill.getSid());
...
//        return resident;
//    }
//    public Resident getResidentDetails(int mygate_no) throws SQLException {
//        Resident resident = new Resident(); // Initialize the resident object
//        boolean residentFound = false; // Flag to check if resident details are found
//
//        try (Connection connection = connectDB()) {
//            String query = "select mem_id, name, room_no, mr_ms, gender, age, contact_no, email, bhk from resident where mygate_no = ?";
//            try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
//                preparedStatement.setInt(1, mygate_no); // Set the 'sid' parameter
//
//                try (ResultSet resultSet = preparedStatement.executeQuery()) {
//                    if (resultSet.next()) { // Check if at least one row is returned
//                        residentFound = true; // Mark that resident details are found
...
//    }
    ...
    public Resident getResident(String mygate_no) throws SQLException {
        ...
        try (Connection connection = connectDB()) {
            connection.setAutoCommit(false);
            String query = "select name, room_no, mr_ms, gender, age, contact_no, email, bhk from resident where mygate_no = ?";
            try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
